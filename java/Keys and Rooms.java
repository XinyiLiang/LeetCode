...
class Solution {
    public boolean canVisitAllRooms(List<List<Integer>> rooms) {
        ...
        HashSet<Integer> visitedRoom = new HashSet<Integer>();
        ...
        while(size != 0)
        {
            ...
            while(i != 0)
            {
                List<Integer> currentRoom = q.poll();
             
                 for(int j=0; j<currentRoom.size(); j++)
                  {
                     if (!visitedRoom.contains(currentRoom.get(j)))
                       {
                          visitedRoom.add(currentRoom.get(j));
                          q.add(rooms.get( currentRoom.get(j) ));
                        }
                   }
                i--;
            }
