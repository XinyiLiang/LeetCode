...
class Solution {
    ...
    public Node copyRandomList(Node head) {
        ...
        ans = readNodes(ans,head);
          
        AddRandom(ans,head);
        return ans;
    }
    
    public Node readNodes(Node answer, Node head)
    {
        if(head == null)
            return answer;
        
        answer = new Node(head.val);
        OriginalNodes.add(head);
        NewNodes.add(answer);
        answer.next = readNodes(answer.next,head.next);
        
         return answer;
    }
    public void AddRandom(Node x, Node head)
    {
        if(head == null)
            return;
        ...
        {
            if(OriginalNodes.indexOf(head.random) != -1)
               x.random =NewNodes.get(OriginalNodes.indexOf(head.random));
        }
        
        
         AddRandom(x.next, head.next);
    }
}
